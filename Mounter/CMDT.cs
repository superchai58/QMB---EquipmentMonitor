    ...
    class CMDT
    {
        ...
        internal string ParsingLog(clsLog log)
        {
            ...
            try
            {
                ...
                List<string> lsMultPath = m_parser.getAllPaths(m_clsLog);

                List<string> lsFileFullPath = new List<string>();

                foreach (string MultPath in lsMultPath)
                {
                    clsCommon.MPath = MultPath;

                    //從資料夾中,取得要處理的檔案名稱
                    lsFileFullPath = m_parser.getDirectoryFile(m_clsLog);
                    //List<string> lsFileFullPath = m_parser.getDirectoryFile(m_clsLog);
                    ...
                    foreach (string sPath in lsFileFullPath)
                    {
                        ...
                        try
                        {
                            ...
                            //回存DB
                            if (!m_parser.saveCMDTDataToDB(m_dtMachineLog, m_clsLog, m_db)) return "";
                        ...
                        finally
                        {
                            ...
                            //if (!clsCommon.BakCMDTFile(ref result))
                            //{
                            //    m_clsLog.writeERRORLog(result);
                            //}
                        }
        ...
        private Boolean ReadFileToTable(string sFilePath)
        {
            ...
            m_parser.InitCMDTDataTable(ref dtTemp);
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    if (i >= m + 2)
                    {
                        ...
                        dtTemp.Rows.Add(splitline[0].Replace("\"", "")+" "+ splitline[1].Replace("\"", ""), splitline[2], splitline[3], splitline[4].Replace("\"", ""), splitline[5].Replace("\"", ""));
                    }
                    i = i + 1;
                }
                ...
                //{
                //    string[] splitline = line.Split(delimiter);
                //    dtTemp.Rows.Add(splitline[0].Replace("#", ""), splitline[2], "", clsCommon.EQ_SN);
                //}
