    ...
    class EM5701N
    {
        ...
        internal string ParsingLog(clsLog log)
        {
            ...
            try
            {
                ...
                //從資料夾中,取得要處理的檔案名稱
                List<string> lsFileFullPath = m_parser.getProcessFileNameByWriteTime("err", m_TimeStamp, m_clsLog);
        ...
        private Boolean ReadFileToTable(string sFilePath)
        {
            ...
            {
                DateTime date;
                string[] lines = File.ReadAllLines(sFilePath, Encoding.GetEncoding("big5"));//big5
                Array.Reverse(lines);
                ...
                foreach (string line in lines)
                {
                    if (line.Contains("/"))
                    {
                        ...
                        string sTime = byline[0].Replace('/', '-') + " " + byline[1];
