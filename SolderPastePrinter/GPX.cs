    ...
    class GPX
    {
        ...
        internal string ParsingLog(clsLog log)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    m_clsLog.writeCOMMLog("ReadFileToTable End");
        ...
        private Boolean ReadFileToTable(string sFilePath)
        {
            ...
            try
            {
                ...
                string sError_Code = "";
                int idx = -1;
                System.IO.StreamReader file = new System.IO.StreamReader(sFilePath);  
                while((line = file.ReadLine()) != null)  
                {
                    string sTime = line.Substring(6, 4) + "-" + line.Substring(3, 2) + "-" + line.Substring(0, 2) + " " + line.Substring(11, 8);
                    string sError = line.Substring(20, line.Length-20);
                    
                    idx= sError.IndexOf("ErrorCode:0x");
                    if (idx > 0)
                    {
                        sError_Code = sError.Substring(idx+12, 8);
                        dtTemp.Rows.Add(sTime, "", sError_Code.ToUpper(),clsCommon.EQ_SN);
                    }                    
                }
                file.Close(); 
