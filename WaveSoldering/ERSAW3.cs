...
namespace EquipmentMonitor.WaveSoldering
{
    class ERSAW3
    {
        ...
        private void GetErrData()
        {
            ...
            //Tag_aufgetreten 開始日期 Zeit_Aufgetreter 開始時間
            //Tag_quittiert 結束日期  Zeit_quittiert 結束時間
            //Betriebsart Auto Model or Maintance Mode,正常是要Auto 生產模式
            string strSQL =
        "SELECT  SWITCH (Tag = NULL, Format(Tag , 'yyyy/MM/dd'),Tag <> NULL, Format(Tag , 'yyyy/MM/dd'))+" +
    "' '+SWITCH (Zeit_quittiert = NULL, Format(Zeit_aufgetreten , 'hh:mm:ss'),Zeit_quittiert <> NULL, Format(Zeit_quittiert , 'hh:mm:ss')) AS [TransDateTime] ," +
    "   Nummer AS [ErrCode], '' AS [ErrMsg], '" + clsCommon.EQ_SN + "' AS EQ_SN " +
    " FROM MonatMeldung " +
    " WHERE (Tag >= @LastParseDate AND Zeit_aufgetreten > @LastParseTime) " +
    "       OR (Tag >= @LastParseDate AND Zeit_quittiert > @LastParseTime ) " +
    "  AND ( Tag <= @NowDate AND Zeit_aufgetreten <= @NowTime ) " +
    ...
    "  AND Betriebsart IN (@Mode_SC,@Mode_EN)";
            ...
            try
            {
                ...
                //oleCmd.Parameters.AddWithValue("@LastParseDate", DbType.DateTime).Value = sTime.Substring(11, 8);
                oleCmd.Parameters.AddWithValue("@LastParseDate", DbType.DateTime).Value = sTime2.Substring(0, 10);
                oleCmd.Parameters.AddWithValue("@LastParseTime", DbType.DateTime).Value = sTime2.Substring(11, 8);
                ...
                //oleCmd.Parameters.AddWithValue("@NowDate", DbType.DateTime).Value = sTime_Now.Substring(11, 8);
                oleCmd.Parameters.AddWithValue("@NowDate", DbType.DateTime).Value = sTime_Now.Substring(0, 10);
                oleCmd.Parameters.AddWithValue("@NowTime", DbType.DateTime).Value = sTime_Now.Substring(11, 8);
