        ...
        internal string ParsingLog(clsLog log)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    //將要處理的資料對應Error Code貼上記號 ---Feix 修改成用ErrorCode作为判断标准
        ...
        private Boolean ReadFileToTable(string sFilePath)
        {
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    string sTime = line.Substring(0, 19).ToString().Replace("/", "-");
                    string sError = line.Substring(line.Length - 4,4);
                    dtTemp.Rows.Add(sTime, sError, sError, clsCommon.EQ_SN);
                }
